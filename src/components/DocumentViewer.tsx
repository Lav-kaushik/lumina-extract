...
const DocumentViewer = ({ fileUrl, fileName, width, onResize }: DocumentViewerProps) => {
  ...
  return (
    <aside style={{ width }} className="relative bg-card border-l border-border flex flex-col">
      <div
        onMouseDown={handleMouseDown}
        className="absolute left-0 top-0 bottom-0 w-1.5 cursor-col-resize hover:bg-primary/30 transition-colors z-10 group"
      >
        <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-muted-foreground/20 rounded-full group-hover:bg-primary/50 transition-colors" />
      </div>

      <div className="h-16 border-b border-border flex items-center px-6 justify-between shrink-0">
      ...
      <div className="flex-1 overflow-hidden m-4 rounded-xl ring-surface bg-muted/30">
        {fileUrl ? (
